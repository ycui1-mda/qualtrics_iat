...
session_state = SessionState.get(
    ...
    working_task=None,
    iat_data=None,
    tools=qualtrics_tools.QualtricsTool()
)
...
def _load_generator():
    ...
    shared_stimuli_instruction = "(words or URLs to images), separated by commas. Enclose each word or URL with single " \
                                 "or double quotes and your entire list with square brackets for coding purposes."
    ...
    if working_task.counter_balancing:
        ...
        st.markdown("WARNING: The order of the blocks will be fixed for all subjects: congruent first then incongruent.")
    st.markdown("___")
    ...
    working_task.overall_instruction_desktop = instruction_desktop_text_col.text_area(
        "HTTP Text",
        working_task._get_overall_instruction(False),
    ...
    working_task.overall_instruction_mobile = instruction_mobile_text_col.text_area(
        "HTTP Text",
        working_task._get_overall_instruction(True),
    ...
    if working_task.shows_examples:
        ...
        working_task.example_instruction = example_instruction_text_col.text_area(
            "HTTP Text",
            working_task._get_example_instruction(),
    ...
    working_task.reminder_instruction_desktop = reminder_instruction_desktop_text_col.text_area(
        "HTTP Text",
        working_task._get_reminder_instruction(False),
    ...
    working_task.reminder_instruction_mobile = reminder_instruction_mobile_text_col.text_area(
        "HTTP Text",
        working_task._get_reminder_instruction(True),
...
def _load_qualtrics_tools():
    ...
    tools.api_token = sidebar.text_input("API Token", "")
    tools.data_center = sidebar.text_input("Data Center", "")
    tools.brand_center = sidebar.text_input("Brand Center", "")
    ...
    library_id = upload_section.text_input("Library ID #", "UR_8B3DdCzUvOPo9Gl")
    ...
    if upload_button and image_files:
        image_urls = tools.upload_images_web(
            image_files,
            library_id,
            full_url,
            qualtrics_folder,
    ...
    if create_button and json_text:
        created_survey_id = tools.create_survey(json_text)
        created_survey_link = f"{tools.base_url}/Q/EditSection/Blocks?ContextSurveyID={created_survey_id}"
    ...
    survey_id = export_section.text_input("Survey ID #", "SV_6V9P5tkOC5WCKRn")
    ...
    if export_button:
        export_content = tools.export_responses(survey_id, file_format)
        create_downloadable_link(
            export_content,
    ...
    delete_library_id = delete_section.text_input("Library ID #", "UR_8B3DdCzUvOPo9Gl", key="for_deletion")
    ...
    if delete_button and delete_image_ids:
        ...
        if isinstance(delete_image_ids, list):
            delete_report = tools.delete_images(delete_library_id, delete_image_ids)
    ...
    if delete_survey_button and delete_survey_id and delete_confirm_text == "Delete Survey":
        delete_survey_report = tools.delete_survey(delete_survey_id)
...
def _load_sidebar():
    ...
    tools = ["IAT Generator", "Qualtrics Tools", "IAT Data Scorer"]
    selected_tool = sidebar.radio("Choose Tool", tools, 0)
    if selected_tool == tools[0]:
        _load_generator()
    elif selected_tool == tools[1]:
